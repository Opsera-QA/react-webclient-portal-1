      ...
      if (logData.data.length > 0) {
        if (logData.data[item].api_response) {
          if (logData.data[item].api_response.jenkinsConsoleLog) {
            completeInput.push(
              logData.data[item].api_response.jenkinsConsoleLog,
            );
          } else if (logData.data[item].api_response.jenkins_console_log) {
            completeInput.push(
              logData.data[item].api_response.jenkins_console_log,
            );
          } else if (
            logData.data[item].step_configuration?.topic ===
            "opsera.pipeline.octopus.console.log"
          ) {
            ...
            if (logData.data[item].api_response) {
              completeInput.push(
                JSON.stringify(logData.data[item].api_response, undefined, 4),
              );
            }
          }
        }
  ...
  const getStatusIcon = (item) => {
    switch (item?.current?.status) {
      case "pending":
        return (
          <IconBase
            className={"float-right mr-2 mt-1 yellow"}
      ...
      case "success":
        return (
          <IconBase
            className={"float-right mr-2 mt-1 green"}
            icon={faCheckCircle}
          />
        );
      default:
        return (
          <IconBase
            className={"float-right mr-2 mt-1 cell-icon red"}
            icon={faTimesCircle}
          />
        );
    }
        ...
        <>
          <div className="ml-1 bordered-content-block px-3 py-2">
            <Row>
              <Col>
                <strong>
                  <span className="blueprint-step-title">
                    {makeUpper(item.start.step_type)}
                  </span>
                </strong>
                {getStatusIcon(item)}
              </Col>
            ...
            {item?.start?.step_name === "approval" ? (
              ...
              <>
                {getRow(
                  "Requested At",
                  format(
                    new Date(item?.current?.timestamp),
                    "yyyy-MM-dd', 'hh:mm:ss a",
                  ),
                )}
    ...
    return (
      <div className="mt-3 bordered-content-block p-3">
        <Row>
          <Col>
            <strong>
              <div className="blueprint-title">{logData?.title}</div>
            </strong>
          </Col>
          {!closeModal && (
            <Button
        ...
        <Row className="mt-1">
          <Col lg>
            <StandaloneTextFieldBase
              label={"Pipeline ID"}
              text={"" + logData?.pipelineId}
            />
          </Col>
          <Col lg>
            <StandaloneTextFieldBase
              label={"Run Number"}
              text={"" + logData?.runNumber}
            />
          </Col>
          <Col
            lg
            className="my-2"
          >
            <span className="text-muted mr-1">Number of Steps:</span>{" "}
            {logData?.data?.length ? logData?.data?.length : "N/A"}
          </Col>
        </Row>
      </div>
    );
                    ...
                    <div className={"d-flex"}>
                      ...
                      {!closeModal && completeInput && (
                        <ExportBlueprintLogButton
                          ...
                          pipelineId={
                            logData?.data[0]?.pipeline_id
                              ? logData.data[0].pipeline_id
                              : "N/A"
                          }
                          runCount={
                            logData?.data[0]?.run_count
                              ? logData.data[0].run_count
                              : "N/A"
                          }
                          blueprintName={
                            logData?.title ? logData?.title : "N/A"
                          }
                          numberOfSteps={
                            logData?.data?.length
                              ? logData?.data?.length
