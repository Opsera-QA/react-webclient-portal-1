...
export default function CreateBitbucketToolButton(
  {
    gitToolModel,
    gitToolId,
    setGitToolId,
  ...
  const saveConnectionDetails = async (toolId) => {
    try {
      ...
      if (isMounted?.current === true) {
        toastContext.showInlineErrorMessage(error, "Error Saving Bitbucket Tool Details:");
      }
    }
  };
  ...
  const handleGitToolCreation = async () => {
    ...
    if (isMongoDbId(gitToolId) !== true) {
      ...
      const newTool = {
        name: `Bitbucket Tool ${currentToolCount + 1}`,
        tool_identifier: toolIdentifierConstants.TOOL_IDENTIFIERS.BITBUCKET,
...
CreateBitbucketToolButton.propTypes = {
