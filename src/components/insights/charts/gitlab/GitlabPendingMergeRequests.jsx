...
function GitlabPendingMergeRequests({ date, tags }) {
  ...
  async function fetchData() {
    ...
    const postBody = {
      request: "gitlabPendingMergeRequests",
    ...
    try {
      ...
      setData(dataObject);
      setLogCount(dataObject.gitlabPendingMergeRequests.data.length);
      setLoading(false);
    } catch (err) {
      setErrors(err);
      setLoading(false);
  ...
  return (
    <>
      {typeof data !== "object" ||
      data.gitlabPendingMergeRequests === undefined ||
      data.gitlabPendingMergeRequests.status !== 200 ? (
        <>
