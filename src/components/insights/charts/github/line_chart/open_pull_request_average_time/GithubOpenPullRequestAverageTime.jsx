...
import config from "./GithubOpenPullRequestAverageTimeChartConfigs.js";
...
import GithubOpenPullRequestAverageTimeDataBlock from "./GithubOpenPullRequestAverageTimeDataBlock";
import IconBase from "components/common/icons/IconBase";
import { faSquare } from "@fortawesome/pro-solid-svg-icons";
import _ from "lodash";
import { smartTimeFormatter } from "components/common/helpers/date/date.helpers";
import GithubOpenPullRequestStatisticsOverlay from "components/insights/charts/github/line_chart/open_pull_request_average_time/GithubOpenPullRequestStatisticsOverlay";
function GithubOpenPullRequestAverageTime({
  kpiConfiguration,
  setKpiConfiguration,
  dashboardData,
  index,
  ...
  const onNodeSelect = (node) => {
    toastContext.showOverlayPanel(
      <GithubOpenPullRequestStatisticsOverlay
        data={node}
  ...
  const loadData = async (cancelSource = cancelTokenSource) => {
    try {
      ...
      const response = await chartsActions.githubOpenPullRequestAverageTime(
        kpiConfiguration,
        getAccessToken,
        cancelSource,
        dashboardTags,
      ...
      let dataObject =
        response?.data?.githubPullRequestAverageOpenTime?.data[0]?.chartData;
      let datablock =
        response?.data?.githubPullRequestAverageOpenTime?.data[0]
          ?.statisticsData;
          ...
          <Row className={"pb-2"}>
            <Col>
              <GithubOpenPullRequestAverageTimeDataBlock
                ...
                lastScore={
                  smartTimeFormatter(dataBlockValues[0]?.prevData, "days")
                    ?.formattedString
                }
      ...
      return (
        ...
        >
          <div style={{ float: "right", fontSize: "10px", marginRight: "5px" }}>
            Average Time Open Requests are Pending{" "}
            <IconBase
              icon={faSquare}
              iconColor={
...
GithubOpenPullRequestAverageTime.propTypes = {
...
export default GithubOpenPullRequestAverageTime;
