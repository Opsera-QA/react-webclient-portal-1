...
import config from "./metricbeatOutNetworkTrafficByTimeLineChartConfigs";
  ...
  const loadData = async (cancelSource) => {
    try {
      ...
      if (envIsProd || envIsTest) {
        const response = await chartsActions.getLegacyChartData(getAccessToken, cancelSource,"OutNetworkTrafficByTime", "line", date);
        const dataObject = response?.data?.data[0]?.OutNetworkTrafficByTime || [];
        setData(dataObject);
      }
      else {
        const postBody = {
        ...
        const dataObject = response?.data?.data[0]?.metricbeatOutNetworkTrafficByTime || [];
        setData(dataObject);
        setLoading(false);
      }
    } catch (err) {
          ...
          <ResponsiveLine
            ...
            tooltip={({ point, color }) => (
              ...
              >
                ...
                <br></br>
                <strong style={{ color }}> Out Network usage: </strong> {point.data.y + "MB"}
              </div>
            )}
            theme={{
              tooltip: {
