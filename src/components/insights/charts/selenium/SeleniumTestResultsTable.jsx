...
function SeleniumTestResultsTable({ kpiConfiguration, setKpiConfiguration, dashboardData, index, setKpis }) {
  ...
  const columns = useMemo(
    () => [
      ...
      getTableTextColumn(getField(fields, "testsSkipped")),
  ...
  const loadData = async (cancelSource = cancelTokenSource, filterDto = tableFilterDto) => {
    try {
      ...
      let dataObject = response?.data?.data[0]?.seleniumTestResults?.data;

      if (isMounted?.current === true && dataObject) {
        setMetrics(dataObject);
        let newFilterDto = filterDto;
        newFilterDto.setData("totalCount", response?.data?.data[0]?.seleniumTestResults?.count);
...
SeleniumTestResultsTable.propTypes = {
...
export default SeleniumTestResultsTable;
