...
function SdlcQualityScanDurationMetric({ kpiConfiguration, dashboardData, meanData, countData, goalsData, metric }) {
  const getDataBlock = () => {
    const qualityScanDataPoint = dataPointHelpers.getDataPoint(kpiConfiguration?.dataPoints,
      constants.SUPPORTED_DATA_POINT_IDENTIFIERS.QUALITY_SCAN_DURATION_DATA_POINT);
    return (
      <SdlcDurationByStageDataBlockBase
        ...
        countData={countData}
        goalsData={goalsData}
        dataPoint={qualityScanDataPoint}
      />
    );
  };
