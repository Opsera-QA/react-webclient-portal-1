...
function JMeterResponseTimeLineChart({ kpiConfiguration, setKpiConfiguration, dashboardData, index, setKpis }) {
  ...
  const loadData = async (cancelSource = cancelTokenSource) => {
    try {
      ...
      const response = await chartsActions.parseConfigurationAndGetChartMetrics(getAccessToken, cancelSource, "jmeterResponseTimeLineChart", kpiConfiguration, dashboardTags);
      const dataObject = response?.data && response?.data?.data[0]?.jmeterResponseTimeLineChart.status === 200 ? response?.data?.data[0]?.jmeterResponseTimeLineChart?.data : [];
