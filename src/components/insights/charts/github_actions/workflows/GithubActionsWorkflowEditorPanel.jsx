...
function GithubActionsWorkflowEditorPanel({
  metricModel,
  unpackedFilterData,
  metricFilterModel,
  ...
  const [hierarchyFiltersDto, setHierarchyFiltersDto] = useState(
    new Model(
      { ...hierarchyFiltersMetadata.newObjectFields },
      hierarchyFiltersMetadata,
      false,
    ),
  );
  ...
  const getFilters = async (cancelSource = cancelTokenSource) => {
    setHierarchyFiltersDto(
      new Model(
        metricModel.getData("filters")[
          metricModel
            .getData("filters")
            .findIndex((obj) => obj.type === "hierarchyFilters")
        ]?.value,
        hierarchyFiltersMetadata,
        false,
      ),
...
GithubActionsWorkflowEditorPanel.propTypes = {
...
export default GithubActionsWorkflowEditorPanel;
