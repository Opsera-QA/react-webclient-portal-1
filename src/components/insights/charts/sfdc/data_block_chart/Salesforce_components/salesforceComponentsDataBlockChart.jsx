...
function SalesforceComponentsDataBlockChart({
  kpiConfiguration,
  setKpiConfiguration,
  dashboardData,
  index,
  ...
  const loadDataPoints = async () => {
    ...
    const salesforceTotalComponentsDataPoint = dataPointHelpers.getDataPoint(
      dataPoints,
    ...
    const totalExecutionsDataPoint = dataPointHelpers.getDataPoint(
      dataPoints,
      dataPointConstants.SUPPORTED_DATA_POINT_IDENTIFIERS
        .SALESFORCE_TOTAL_EXECUTIONS_DATA_POINT,
    );
    setTotalSalesforceDatapoint(totalExecutionsDataPoint);
    const salesforceDeploymentDataPoint = dataPointHelpers.getDataPoint(
      dataPoints,
      dataPointConstants.SUPPORTED_DATA_POINT_IDENTIFIERS
        .SALESFORCE_DEPLOYMENTS_DATA_POINT,
    );
    setDeploymentDatapoint(salesforceDeploymentDataPoint);
    const salesforceValidationDataPoint = dataPointHelpers.getDataPoint(
      dataPoints,
      dataPointConstants.SUPPORTED_DATA_POINT_IDENTIFIERS
        .SALESFORCE_VALIDATION_DATA_POINT,
    );
    setValidationDatapoint(salesforceValidationDataPoint);
    const salesforceUnitTestsDataPoint = dataPointHelpers.getDataPoint(
      dataPoints,
      dataPointConstants.SUPPORTED_DATA_POINT_IDENTIFIERS
        .SALESFORCE_UNIT_TESTS_DATA_POINT,
    );
              ...
              <Col className={"mr-5"}>
                <TotalComponentsDeployed
                  ...
                  className={getIconColor(
                    metrics[0]?.trends?.totalComponentsDeployed,
                  )}
                  lastScore={
                    metrics[0]?.previousResults?.totalComponentsDeployed
                  }
            ...
            {dataPointHelpers.isDataPointVisible(uniqueDataPoint) && (
              <Col>
                <AvgComponentsDeployedPerExecution
                  score={
                    metrics[0]?.currentResults?.totalUniqueComponentsDeployed
                      ? metrics[0]?.currentResults
                          ?.totalUniqueComponentsDeployed
                      : 0
                  }
                  icon={getIcon(
                  ...
                  lastScore={
                    metrics[0]?.previousResults?.totalUniqueComponentsDeployed
                  }
                  iconOverlayBody={getDescription(
                    metrics[0]?.trends?.totalUniqueComponentsDeployed,
                  )}
            ...
            {dataPointHelpers.isDataPointVisible(totalSalesforceDataPoint) && (
              <Col className={"ml-5"}>
                <TotalSalesforcePipelineExecutions
                  ...
                  icon={getIcon(
                    metrics[0]?.trends?.totalSalesforcePipelineExecutions,
                  )}
                  className={getIconColor(
                    metrics[0]?.trends?.totalSalesforcePipelineExecutions,
                  )}
                  lastScore={
                    metrics[0]?.previousResults
                      ?.totalSalesforcePipelineExecutions
                  }
                  iconOverlayBody={getDescription(
                    metrics[0]?.trends?.totalSalesforcePipelineExecutions,
            ...
            {dataPointHelpers.isDataPointVisible(deploymentDataPoint) && (
              <Col className={"mr-5"}>
                <TotalPipelineExecutionDeployment
                  score={
                    metrics[0]?.currentResults
                      ?.totalPipelineExecutionsWithDeployment
                      ? metrics[0]?.currentResults
                          ?.totalPipelineExecutionsWithDeployment
                      : 0
                  }
                  icon={getIcon(
                    metrics[0]?.trends?.totalPipelineExecutionsWithDeployment,
                  )}
                  className={getIconColor(
                    metrics[0]?.trends?.totalPipelineExecutionsWithDeployment,
                  )}
            ...
            {dataPointHelpers.isDataPointVisible(validationDataPoint) && (
              <Col>
                <TotalPipelineExecutionValidation
                  score={
                    metrics[0]?.currentResults
                      ?.totalPipelineExecutionsWithValidation
                      ? metrics[0]?.currentResults
                          ?.totalPipelineExecutionsWithValidation
                      : 0
                  }
                  icon={getIcon(
                    metrics[0]?.trends?.totalPipelineExecutionsWithValidation,
                  )}
                  className={getIconColor(
                    metrics[0]?.trends?.totalPipelineExecutionsWithValidation,
                  )}
            ...
            {dataPointHelpers.isDataPointVisible(unitTestsDataPoint) && (
              <Col className={"ml-5"}>
                <TotalPipelinesExecutionsUnitTests
                  score={
                    metrics[0]?.currentResults
                      ?.totalPipelineExecutionsWithUnitTests
                      ? metrics[0]?.currentResults
                          ?.totalPipelineExecutionsWithUnitTests
                      : 0
                  }
                  icon={getIcon(
                    metrics[0]?.trends?.totalPipelineExecutionsWithUnitTests,
                  )}
                  className={getIconColor(
                    metrics[0]?.trends?.totalPipelineExecutionsWithUnitTests,
                  )}
