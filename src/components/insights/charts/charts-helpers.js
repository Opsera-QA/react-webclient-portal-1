...
export function getDateObjectFromKpiConfiguration(kpiConfiguration) {
  ...
  if (date?.label) {
    ...
    return {
      ...
      end: addDays(new Date(date.endDate), 1).toISOString(),
    };
  }
  return {
    start: new Date(
...
export function getResultFromKpiConfiguration(kpiConfiguration, filter) {
  if (
    ...
    kpiConfiguration?.filters[
      kpiConfiguration.filters.findIndex((obj) => obj.type === filter)
    ]?.value &&
    kpiConfiguration?.filters[
      kpiConfiguration.filters.findIndex((obj) => obj.type === filter)
    ]?.value.length > 0
...
export function getGitlabExcludedUsersFromKpiConfiguration(kpiConfiguration) {
  return getResultFromKpiConfiguration(
    kpiConfiguration,
...
export function getJiraProjectsFromKpiConfiguration(kpiConfiguration) {
  if (
    kpiConfiguration?.filters[
      kpiConfiguration.filters.findIndex((obj) => obj.type === "jira-projects")
    ]?.value &&
    kpiConfiguration?.filters[
      kpiConfiguration.filters.findIndex((obj) => obj.type === "jira-projects")
    ]?.value.length > 0
...
export const getTimeDisplay = (mins) => {
  ...
  if (remainingSeconds > 0) {
    arrayToDisplay.push(
      remainingSeconds + (remainingSeconds === 1 ? " sec" : " sec"),
    );
  }
...
export function getCustomMappingFields(kpiConfiguration) {
  if (
    ...
    kpiConfiguration?.dataPoints?.length > 0 &&
    kpiConfiguration?.dataPoints.filter(
      (point) =>
        point?.customFieldsMapping?.enabled &&
        point?.customFieldsMapping?.mappedFields,
    ).length > 0
  ) {
    let customFields = {};
