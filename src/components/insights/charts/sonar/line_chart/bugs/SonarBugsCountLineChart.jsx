...
function BugsCountLineChart({ kpiConfiguration, setKpiConfiguration, dashboardData, index, setKpis }) {
  ...
  const loadData = async (cancelSource = cancelTokenSource) => {
    try {
      ...
      const response = await chartsActions.parseConfigurationAndGetChartMetrics(getAccessToken, cancelSource, "bugs", kpiConfiguration, dashboardTags);
      const dataObject = response?.data && response?.data?.data[0]?.bugs.status === 200 ? response?.data?.data[0]?.bugs?.data : [];
