...
import ConnectedAssetsPackagesPipelinesTable from "./ConnectedAssetsPackagesPipelinesTable";
import PaginationContainer from "../../../../../common/pagination/PaginationContainer";
import { CONNECTED_ASSETS_CONSTANTS as constants } from "../../../connecetdAssets.constants";

function ConnectedAssetsPackagesPipelinesTab({ dashboardData }) {
  ...
  const loadData = async (cancelSource = cancelTokenSource, filterDto = tableFilterDto) => {
    try {
      ...
      const response = await connectedAssetsActions.getListOfRepositories(
        getAccessToken,
        cancelSource,
        constants.PACKAGES_LIST.PIPELINE_LIST,
        dateRange?.startDate,
        dateRange?.endDate,
        filterDto
      );
      const responseData1 = response?.data?.data?.pipelinesList?.data?.[0];
    ...
    return (
      <>
        <VanitySetTabAndViewContainer
          className={"mb-3"}
          title={`List of Packages from Pipelines`}
  ...
  const getVerticalTabContainer = () => {
    if(!responseData || responseData.length === 0) {
      return (<div className={"h-100"}>
        <VanitySetVerticalTabContainer className={"h-100"} title={<div><IconBase icon={faDiagramSuccessor} className={'pr-2'}/>List Of Pipelines</div>}>
    ...
    for(let i = 0; i <= responseData.length - 1; i++) {
      tabs.push(
        <VanitySetVerticalTab
          tabText={responseData[i]?.pipeline_name}
...
ConnectedAssetsPackagesPipelinesTab.propTypes = {
  dashboardData: PropTypes.object,
};

export default ConnectedAssetsPackagesPipelinesTab;
