...
import {faCircleInfo, faCodeBranch} from "@fortawesome/pro-solid-svg-icons";
...
function ConnectedAssetsBranchesTasksTable({ repository, dashboardData, icon }) {
  ...
  const loadOpenData = async (cancelSource = cancelTokenSource, filterDto = filterModel) => {
    ...
    const response = await connectedAssetsActions.getSelectedRepoDetailedInfo(
      getAccessToken,
      cancelSource,
      constants.BRANCHES_LIST.SELECTED_REPO_TASKS_INFO,
      dateRange?.startDate,
      dateRange?.endDate,
      filterDto,
      repo
    );
    let dataObject = response?.data?.data?.branchInfoForTask?.data?.[0];
...
ConnectedAssetsBranchesTasksTable.propTypes = {
  repository: PropTypes.object,
  dashboardData: PropTypes.object,
  kpiConfiguration: PropTypes.object,
  icon: PropTypes.object
};
export default ConnectedAssetsBranchesTasksTable;
