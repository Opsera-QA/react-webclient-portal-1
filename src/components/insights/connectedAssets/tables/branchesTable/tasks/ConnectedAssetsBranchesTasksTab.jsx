  ...
  const loadData = async (cancelSource = cancelTokenSource, filterDto = tableFilterDto) => {
    try {
      ...
      const response = await connectedAssetsActions.getListOfRepositories(
        getAccessToken,
        cancelSource,
        constants.BRANCHES_LIST.REPOSITORIES_LIST_FROM_TASKS,
        dateRange?.startDate,
        dateRange?.endDate,
        filterDto
      );
    ...
    return (
      <>
        <VanitySetTabAndViewContainer
          className={"mb-3"}
          title={`List of Branches from Tasks`}
...
ConnectedAssetsBranchesTasksTab.propTypes = {
  dashboardData: PropTypes.object,
};

export default ConnectedAssetsBranchesTasksTab;
