...
import {faCircleInfo, faDiagramSuccessor} from "@fortawesome/pro-solid-svg-icons";
...
function ConnectedAssetsPipelinesInactiveTable({ dashboardData }) {
  ...
  const loadRecentPipelineData = async (cancelSource = cancelTokenSource, filterDto = filterModel) => {
    ...
    const response = await connectedAssetsActions.getPipelinesInfo(
      getAccessToken,
      cancelSource,
      constants.PIPELINES_LIST.INACTIVE_PIPELINES,
      dateRange?.startDate,
      dateRange?.endDate,
      filterDto,
    );
  ...
  return (
    <div className={"p-2"}>
      ...
      <FilterContainer
        isLoading={isLoading}
        title={'List Of Inactive Pipelines'}
...
ConnectedAssetsPipelinesInactiveTable.propTypes = {
  dashboardData: PropTypes.object,
};
export default ConnectedAssetsPipelinesInactiveTable;
