...
function GitTasksActivityLogsTable({ gitTasksData, allLogs }) {
  ...
  const loadData = async (filterDto = gitTasksActivityFilterDto) => {
    try {
      ...
      if(allLogs) {
        gitTasksLogResponse = await taskActions.getAllGitTasksActivityLogs(filterDto, getAccessToken);
      } else {
        gitTasksLogResponse = await taskActions.getGitTaskActivityLogs(gitTasksData, filterDto, getAccessToken);
      }
