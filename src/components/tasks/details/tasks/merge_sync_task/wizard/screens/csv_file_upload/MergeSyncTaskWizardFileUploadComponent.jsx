...
import MergeSyncWizardFileUploadMetadata from "components/tasks/details/tasks/merge_sync_task/wizard/screens/csv_file_upload/merge-sync-wizard-file-upload-metadata";
import SaveButtonContainer from "components/common/buttons/saving/containers/SaveButtonContainer";
import MergeSyncTaskWizardSubmitFileTypeButton
  from "components/tasks/details/tasks/merge_sync_task/wizard/screens/csv_file_upload/MergeSyncTaskWizardSubmitFileTypeButton";
...
function MergeSyncTaskWizardFileUploadComponent({ wizardModel, setWizardModel, setWizardScreen, handleClose }) {
  const fields = MergeSyncWizardFileUploadMetadata.fields;
  ...
  const getXMLView = () => {
    if (wizardModel.getData("xmlFileContent") && wizardModel.getData("xmlFileContent").length > 0) {
      return (
        <>
          <PackageXmlFieldBase fieldName={"xmlFileContent"} model={wizardModel} />
          {buttonContainer()}
        </>
      );
    }
  ...
  const buttonContainer = () => {
    return (
      <SaveButtonContainer>
        <MergeSyncTaskWizardSubmitFileTypeButton
          wizardModel={wizardModel}
          setWizardScreen={setWizardScreen}
          isXml={wizardModel?.getData("isXml")}
        />
  ...
  const getFileUploadBody = () => {
    if (wizardModel.getData("xmlFileContent").length === 0 && wizardModel.getData("csvFileContent").length === 0) {
      return (
  ...
  const getValidateButton = () => {
    if (unsupportedFiles.length === 0 && validFiles.length && wizardModel.getData("xmlFileContent").length === 0 && wizardModel.getData("csvFileContent").length === 0) {
      return (
...
MergeSyncTaskWizardFileUploadComponent.propTypes = {
...
export default MergeSyncTaskWizardFileUploadComponent;
