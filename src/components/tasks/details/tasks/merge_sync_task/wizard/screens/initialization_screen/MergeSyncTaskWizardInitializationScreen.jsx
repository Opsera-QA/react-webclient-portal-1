...
const MergeSyncTaskWizardInitializationScreen = ({
  ...
  const getTabContainer = () => {
    return (
      <div>
        <div className={"mt-2"}>
          {wizardModel?.getData("taskType") === "SFDC_GIT_COMPARE_SYNC" ? `Would you like to start a manual ${mergeSyncType} wizard run or use the XML/File Upload Process?` : `Would you like to start a manual ${mergeSyncType} wizard run?`}
        </div>
        <div className={"mt-2"}>
          <CustomTabContainer>
            <CustomTab
            ...
            <CustomTab
              ...
              icon={faFileCode}
              visible={wizardModel?.getData("taskType") === "SFDC_GIT_COMPARE_SYNC"}
            />
          </CustomTabContainer>
        </div>
      </div>
