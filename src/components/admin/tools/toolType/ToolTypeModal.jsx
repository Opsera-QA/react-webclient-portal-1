...
function ToolTypeModal(props) {
  ...
  const updateToolType = async () => {
    ...
    try {
      const accessToken = await getAccessToken();
      const response = await axiosApiService(accessToken).post("/registry/type/"+ props.toolId + "/update", { ...formData });
      setToolData(response.data);
      setToolType("View");
    }
    catch (err) {
  ...
  const deleteTool = async () => {
    try {
      const accessToken = await getAccessToken();
      const response = await axiosApiService(accessToken).delete("/registry/type/"+ props.toolId, { });
