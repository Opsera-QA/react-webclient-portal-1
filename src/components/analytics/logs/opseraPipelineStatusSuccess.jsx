...
// import { faCalendar, faDraftingCompass, faDownload } from "@fortawesome/pro-solid-svg-icons";
...
function OpseraPipelineStatusSuccess({ persona, date }) {
  ...
  const fetchData = async () => {
    ...
    const postBody = {
      data: [
        {
          request: "opseraPipelineInfoSuccess",
          metric: "bar",
        },
      ],
    ...
    try {
      ...
      let dataObject = res && res.data ? res.data.data[0].opseraPipelineInfoSuccess.data : [];
      setData(dataObject);
      setLoading(false);
    } catch (err) {
  ...
  return (
    <>
      <div className="activity-label-text mb-2">Recent Successful OpsERA Pipelines</div>
...
OpseraPipelineStatusSuccess.propTypes = {
