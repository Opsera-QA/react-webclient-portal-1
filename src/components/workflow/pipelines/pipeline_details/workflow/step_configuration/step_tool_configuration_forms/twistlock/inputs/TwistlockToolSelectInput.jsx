...
function TwistlockToolSelectInput({ fieldName, dataObject, setDataObject, disabled, textField, valueField, tool_prop }) {
  const toastContext = useContext(DialogToastContext);
  const { getAccessToken } = useContext(AuthContext);
  const [twistlockList, setTwistlockList] = useState([]);
  const [isTwistlockSearching, setIsTwistlockSearching] = useState(false);
  ...
  return (
    <div>
      <SelectInputBase
        ...
        selectOptions={twistlockList ? twistlockList : []}
        busy={isTwistlockSearching}
        valueField={valueField}
        textField={textField}
        placeholderText={placeholder}
        disabled={disabled || isTwistlockSearching || (twistlockList == null || twistlockList.length === 0)}
      />
...
TwistlockToolSelectInput.propTypes = {
...
TwistlockToolSelectInput.defaultProps = {
  valueField: "id",
  textField: "name",
  fieldName: "twistlockToolId",
};

export default TwistlockToolSelectInput;
