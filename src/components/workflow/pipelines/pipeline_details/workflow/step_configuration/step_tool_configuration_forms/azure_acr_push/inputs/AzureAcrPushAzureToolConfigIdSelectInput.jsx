...
function AzureAcrPushAzureToolConfigIdSelectInput({ fieldName, dataObject, setDataObject, disabled, textField, valueField , setAzureConfig}) {
  const toastContext = useContext(DialogToastContext);
  const { getAccessToken } = useContext(AuthContext);
  const [list, setList] = useState([]);
  const [isSearching, setIsSearching] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [placeholder, setPlaceholder] = useState("Select a Azure Acr Push Instance");
  ...
  const fetchAzureDetails = async (cancelSource) => {
    ...
    try {
      ...
      setPlaceholder("No Azure Acr Push Tool Found");
      console.error(error);
