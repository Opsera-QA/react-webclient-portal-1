...
function CSVFileUploadComponent({
  ...
  const validateObj = (obj) => {
    ...
    let validationObj = [];
    if (fromSFDC) {
      validationObj = allSFDCComponentType; 
    }
    if (fromDestinationSFDC) {
      validationObj = allDestSfdcComponentType; 
    }
    if (fromGit) {
      validationObj = allGitComponentType; 
    }
    if(fromProfileComponents){
      validationObj = allProfileComponentType;
    }

    let matched = validationObj.filter(o => obj.some(({componentType,committedFile}) => o.componentType === componentType && o.committedFile === committedFile));
    // console.log(matched); 
  ...
  return (
      <>
      <div className="page-description px-3 py-2">Select the components or upload components as a csv file to use for deployment.</div>
      {error &&
        <div className="mt-3">
            ...
            <div className="file-display-container">
                {
                  validFiles.map((data, i) => 
                    <div className="file-status-bar" key={i}>
                        ...
                        <div className="file-remove" onClick={() => removeFile(data.name)}><i className="fa fa-trash" aria-hidden="true"></i></div>
                    </div>
                  )
                }
            </div>
            {unsupportedFiles.length === 0 && validFiles.length && matchedItems.length === 0 && rejectedItems.length === 0 ? <Button variant="primary" onClick={() => validateFiles()}>Validate File</Button> : ''}
            {matchedItems.length > 0 || rejectedItems.length > 0 ? (
            <>
              <Row>
                <Col xs={6}>
