...
function BitbucketToolConfiguration({ toolData, toolId, fnSaveChanges, fnSaveToVault }) {
  ...
  return (
    <div>
      <Row>
        <div className="ml-auto"><TestToolConnectionButton recordData={toolData} toolName={"Bitbucket"} disable={isNew}/></div>
      </Row>
      <Form>
        {showToast && toast}
        ...
        <>
          <Form.Group controlId="password">
            ...
            <Form.Control as="textarea" type="password" placeholder="" style={{WebkitTextSecurity: 'disc'}}  value={formData.secretPrivateKey || ""} onChange={e => setFormData({ ...formData, secretPrivateKey: e.target.value.trim() })} />
            <Form.Text className="text-muted">These credentials will be securely stored in vault.</Form.Text>
          </Form.Group>
