...
function JenkinsAccountSecretFileUploadInput({ fieldName, model, setModel, setDataFunction, clearDataFunction, disabled, isLoading }) {
  ...
  const handleFile = () => {
    ...
    if (file != null && validateFileSize(file)) {
      ...
      reader.onload = async (evt) => {
        ...
        if (parsedFile) {
          const unsignedIntArray = new Uint8Array(reader.result);
          const arrayValue = Array.from(unsignedIntArray);
          await validateAndSetData(arrayValue, file?.name);
        }
      };      
...
JenkinsAccountSecretFileUploadInput.propTypes = {
...
export default JenkinsAccountSecretFileUploadInput;
