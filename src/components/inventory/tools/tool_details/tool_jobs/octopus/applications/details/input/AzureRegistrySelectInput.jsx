...
function AzureAcrPushRegistryNameSelectInput({
  ...
  const loadData = async (cancelSource = cancelTokenSource) => {
    try {
      ...
      setErrorMessage("No Registries available.");
  ...
  const loadAzureRegistries = async (cancelSource = cancelTokenSource) => {
    const response = await octopusActions.getAzureRegistries(
      getAccessToken,
      cancelSource,
      azureConfig,
      applicationData
    ...
    if (result?.length === 0) {
      setPlaceholderText("No registries found with this azure configuration");
      setErrorMessage("No Azure Registries have been found associated with this Azure Account");
    }
  };
