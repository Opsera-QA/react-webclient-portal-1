...
function AzureToolSelectInput({ fieldName, dataObject, setDataObject, disabled, textField, valueField, tool_prop}) {
  ...
  const fetchAWSDetails = async () => {
    setIsAzureSearching(true);
    try {
      let results = await PipelineActions.getToolsList("azure_account", getAccessToken);
  ...
  return (
    <div>
      <SelectInputBase
        ...
        selectOptions={azureList ? azureList : []}
        busy={isAzureSearching}
        valueField={valueField}
        textField={textField}
        placeholderText={placeholder}
        disabled={disabled || isLoading || (!isLoading && (azureList == null || azureList.length === 0))}
      />
    </div>
  );
}
