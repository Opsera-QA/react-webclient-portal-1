  ...
  return (
    <div className="px-4 mt-2">
      ...
      {(jobAction === "CREATE_ACCOUNT") && <>
        <JenkinsCreateAccount toolId={toolData._id} toolData={toolData} jobAction={jobAction}
                              setJobAction={(action) => setJobAction(action)}/>
      </>}

      {(jobAction === "CREATE_JOB") && <>
        <JenkinsCreateJob toolId={toolData._id} toolData={toolData} jobAction={jobAction}
                          setJobAction={(action) => setJobAction(action)} jobData={jobData}/>
      </>}

      {(jobAction === "JOB_LIST" || jobAction === "ACCOUNT_LIST") && <> <ButtonToolbar
        className="justify-content-between">
        <ButtonGroup>
          <Button size="sm" className="mr-2" variant={jobAction === "JOB_LIST" ? "primary" : "secondary"}
                  onClick={() => setJobAction("JOB_LIST")}>Jobs</Button>
          <Button size="sm" className="mr-2" variant={jobAction === "ACCOUNT_LIST" ? "primary" : "secondary"}
                  onClick={() => setJobAction("ACCOUNT_LIST")}>Accounts</Button>
        </ButtonGroup>
      </ButtonToolbar>
      </>}
