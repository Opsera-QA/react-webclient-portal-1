    ...
    return (
      <div>Create and manage the following Opsera Tasks. To view in depth documentation on Opsera Tasks Management view the <b><a href="https://opsera.atlassian.net/l/c/cd5ujPjA" target="_blank" rel="noreferrer">Opsera Task Management Help Documentation</a></b> or click a task below for a link to individual task help documentation.
          <div className={"mt-2 ml-4"}>
              <div><b><a href="https://opsera.atlassian.net/l/c/bfaswunv" target="_blank" rel="noreferrer">{getTaskTypeLabel(TASK_TYPES.AWS_CREATE_ECS_CLUSTER)}</a></b> - Create an AWS ECS Cluster template and run the task to create a cluster.</div>
              <div><b><a href="https://opsera.atlassian.net/l/c/xsDLErWs" target="_blank" rel="noreferrer">{getTaskTypeLabel(TASK_TYPES.AWS_CREATE_ECS_SERVICE)}</a></b> - Create Amazon Elastic Container Service to integrate tasks and management, then publish the containers through Opsera pipelines. </div>
              <div><b><a href="https://opsera.atlassian.net/l/c/9hB3DPfP" target="_blank" rel="noreferrer">{getTaskTypeLabel(TASK_TYPES.AWS_CREATE_LAMBDA_FUNCTION)}</a></b> - Create an AWS Lambda Function template and run the task to create a cluster. </div>
              <div><b><a href="https://opsera.atlassian.net/l/c/p1D1mnbo" target="_blank" rel="noreferrer">{getTaskTypeLabel(TASK_TYPES.AZURE_CLUSTER_CREATION)}</a></b> - Create an AKS Azure Cluster template and run the task to create a cluster. </div>
              <div><b><a href="https://opsera.atlassian.net/l/c/ZpdGHtzK" target="_blank" rel="noreferrer">{getTaskTypeLabel(TASK_TYPES.GITSCRAPER)}</a></b> - Choose from Custodian libraries then run a scan against the configured SCM repos. Define a maximum threshold and choose any secrets to ignore in the scan.</div>
              ...
              <div><b><a href="https://opsera.atlassian.net/l/c/wbWoC5yR" target="_blank" rel="noreferrer">{getTaskTypeLabel(TASK_TYPES.GIT_TO_GIT_MERGE_SYNC)}</a></b> - Use this task to selectively merge changes from a source branch to a target branch. </div>
