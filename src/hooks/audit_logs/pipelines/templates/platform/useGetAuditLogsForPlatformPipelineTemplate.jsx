...
import usePlatformPipelineTemplateAuditLogActions from "hooks/audit_logs/pipelines/templates/platform/usePlatformPipelineTemplateAuditLogActions";

export default function useGetAuditLogsForPlatformPipelineTemplate(
  templateId,
  handleErrorFunction,
) {
  ...
  const getAuditLogsForPlatformPipelineTemplate = async (newFilterModel = auditLogFilterModel) => {
    if (isMongoDbId(templateId) !== true) {
      return;
    }
    setAuditLogs([]);

    const response = await platformPipelineTemplateAuditLogActions.g(
      templateId,
